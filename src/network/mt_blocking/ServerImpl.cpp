...
void ServerImpl::Worker(std::list<std::thread>::iterator it, int client_socket) {
        ...
        while ((readed_bytes = read(client_socket, client_buffer, sizeof(client_buffer))) > 0) {
            ...
            while (readed_bytes > 0) {
                ...
                if (!command_to_execute) {
                    std::size_t parsed = 0;
                    if (parser.Parse(client_buffer, static_cast<size_t>(readed_bytes), parsed)) {
