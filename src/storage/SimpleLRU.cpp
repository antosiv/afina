...
bool SimpleLRU::Put(const std::string &key, const std::string &value) {
    ...
    if (match != _lru_index.end()) {
        if (!put_by_match(match, value))
            return false;
    } else {
        if (!put_to_head(key, value))
            return false;
    }
    return true;
}
...
bool SimpleLRU::put_to_head(const std::string &key, const std::string &value) {
    if (!clear_space(value.size() + key.size()))
        return false;
    ...
    _lru_index.emplace(std::make_pair(std::reference_wrapper<const std::string>(_lru_head->key),
...
bool SimpleLRU::put_by_match(Afina::Backend::SimpleLRU::index_it match, const std::string &value) {
    move_to_head(match);
    if (!clear_space(value.size() - match->second.get().value.size()))
        return false;
